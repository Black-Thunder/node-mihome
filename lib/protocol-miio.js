...
class MiIOProtocol extends EventEmitter {
	...
	async handshake(address) {
		...
		const needsHandshake = !device._serverStampTime || Date.now() - this._serverStampTime > 120000;
		if (!needsHandshake) {
		...
		return new Promise((resolve, reject) => {
			...
			const retry = () => {
				...
				if (retriesLeft-- > 0) {
					...
					debug(
						`${address} <- Reached maximum number of retries, giving up ${method} - ${JSON.stringify(params)}`,
					);
