...
const miotSpecCache = require("./miot-spec-cache");
...
module.exports = class MiioDevice extends EventEmitter {
	constructor({ id, address, token, protocol, refresh }) {
		super();
		this.id = String(id);
	...
	async send(method, params, options = {}) {
		const result =
			this.protocol === "cloud"
				? await this.cloudSend(method, params, options)
				: this.localSend(method, params, options);
		return result;
	}
		...
		try {
			...
			for (const propChunk of propsChunks) {
				...
				if (resultChunk.length !== propChunk.length) {
					throw new Error(
						`Result ${JSON.stringify(resultChunk)} and props ${JSON.stringify(propChunk)} does not match length`,
					);
				}
	...
	async getProperties(props) {
		const result = this._miotSpec ? await this.miotGetProperties(props) : await this.miioGetProperties(props);
		return result;
	}
	...
	async miotFetchSpec(spec) {
		if (miotSpecCache[spec]) {
			this._miotSpec = miotSpecCache[spec];
			return this._miotSpec;
		}

		throw new Error(`No local MIOT spec found for ${spec}`);
	}
};
